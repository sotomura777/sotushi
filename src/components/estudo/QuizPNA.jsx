...
function ordemInteligente(dados, historico) {
...
function novaSessao(dados, historico) {
  const ordem = ordemInteligente(dados, historico);
...
export default function QuizPNA({ dados, prefixo, rotuloVoltar = "‹ Voltar", classeTema = "ca", voltar }) {
  ...
  const recomecar = () => { setSessao(novaSessao(dados, historico)); setAtual(0); setRespostas({}); setVista("pergunta"); };
  ...
  return (
    <div className={classeTema + " ob-page"}>
      {!onboardVisto && dados.onboarding && (
      ...
      {dados.disclaimer && <div className="ob-aviso" dangerouslySetInnerHTML={{ __html: dados.disclaimer }} />}
      ...
      {vista === "resultados" && (
        <div className="ca-resultados">
          ...
          <div className="ca-flash-nav">
            ...
            <button className="ca-btn-out" onClick={() => { if (confirm("Apagar todo o histórico de respostas? Os casos voltam ao estado inicial.")) { setHistorico({}); setSessao(novaSessao(dados, {})); setAtual(0); setRespostas({}); setVista("pergunta"); } }}>Apagar histórico</button>
          </div>
        </div>
      )}
