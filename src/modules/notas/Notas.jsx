...
// Cores por tipo de nota (consistentes com as cores do conteúdo).
const KIND = {
...
function NotaCard({ nota, tab, onUpdate, onApagar, onCopiar }) {
  ...
  return (
    <div className="nt-nota" style={{ borderLeft: `3px solid ${cor}` }}>
      <div className="nt-nota-cab" style={{ background: `${cor}14` }}>
        ...
        {antigo ? (
          <input
            type="date"
            value={nota.data}
            onChange={(e) => onUpdate({ ...nota, data: e.target.value })}
            className="campo"
            style={{ width: "auto", padding: "2px 6px", fontSize: 11, background: "transparent" }}
          />
        ) : (
      ...
      {aviso && (
        <div className="nt-aviso-num"><Ico name="warn" s={12} /> Sequência numérica detetada — não inserir identificadores de doente.</div>
      )}
      <textarea
...
export default function Notas({ accent = "#475569", gradiente, onVoltar }) {
  ...
  const abrir = (p) => { setSelId(p.id); setAba(TABS[0].id); setQuery(""); setFixadaId(null); setEditId(null); setView("patient"); };

  // Notas do separador ativo
  ...
  const addComTemplate = () => {
    const tpl = (tab.template || "").replace(/\[DATA\]/g, fmtData(hojeISO()));
    patchNotas(selId, aba, (arr) => [criarNota("normal", tpl), ...arr]);
  };
  const copiarAnterior = () => {
    const prev = ordenarNotas(sel.notas[aba] || [], null, "")[0];
    patchNotas(selId, aba, (arr) => [criarNota("normal", prev ? prev.texto : ""), ...arr]);
  };
  ...
  const hero = (
    <header className="hero" style={{ background: gradiente || accent }}>
      <div className="hero-conteudo">
        <button className="voltar" onClick={view === "patient" ? () => setView("list") : onVoltar}>← {view === "patient" ? "Doentes" : "Início"}</button>
        <div className="hero-titulo">Notas Clínicas</div>
        <div className="hero-subtitulo">{view === "patient" ? "Notas por separador · só iniciais" : "Lista de doentes · só iniciais"}</div>
      </div>
    </header>
  );
    ...
    return (
      <div style={{ "--acento": accent }}>
        ...
        <div className="modulo-corpo">
          {editId === sel.id ? (
            <FormDoente inicial={sel} accent={accent} onGuardar={guardarEdicao} onCancelar={() => setEditId(null)} />
          ) : (
            <div className="cartao" style={{ padding: 14, marginBottom: 12, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <button className="nt-btn" onClick={() => setEditId(sel.id)}>Editar</button>
            ...
            <div key={g.data}>
              ...
              {g.notas.map((n) => (
                <NotaCard
                  ...
                  tab={tab}
                  onUpdate={updateNota}
                  onApagar={() => apagarNota(n.id)}
                  onCopiar={isDiario && n.kind !== "rosa" && n.kind !== "antigo" ? copiarAnterior : null}
                />
              ))}
            </div>
          ))}
  ...
  const cardsDe = (lista) => (
    <div className="nt-grid">
      {lista.map((p) => (
        <CartaoDoente key={p.id} p={p} onSelect={() => abrir(p)} onVisto={setVisto} onAlta={setAlta} onEditar={(pat) => { setAdicionar(false); setEditId(pat.id); }} />
      ))}
    </div>
  );

  const grupo = (label, lista, corLabel) =>
    lista.length > 0 && (
      <div className="nt-grupo">
        <div className="nt-grupo-label" style={corLabel ? { color: corLabel } : undefined}>{label} · {lista.length}</div>
        {cardsDe(lista)}
      </div>
    );
    ...
    <div style={{ "--acento": accent }}>
      ...
      <div className="modulo-corpo">
        <div className="nt-aviso">
          <Ico name="lock" s={13} /> <strong>Pré-visualização.</strong> Nada é guardado ainda — o armazenamento encriptado e a sincronização chegam com o backend. Insere <strong>apenas iniciais</strong>; o nome é convertido automaticamente e o nome completo nunca é guardado. Não inserir dados reais de doentes.
        </div>

        <div className="nt-topo">
          <div>
            <div className="nt-h1">Doentes</div>
            <div className="nt-count">{ativos.length} ativo{ativos.length !== 1 ? "s" : ""}</div>
          </div>
        ...
        {adicionar && <FormDoente accent={accent} onGuardar={criarDoente} onCancelar={() => setAdicionar(false)} />}
        ...
        {sidebarView === "doentes" ? (
          <>
            {grupo("Urgência", urgencia, "#8B1A3A")}
            {grupo("Internados", internados)}
            {grupo("Vistos / Altas", fora)}
          </>
        ) : (
          porEspecialidade.map((s) => (
            <div key={s.id} className="nt-grupo">
              <div className="nt-grupo-label"><span className="nt-mini-dot" style={{ background: s.color }} />{s.label} · {s.pts.length}</div>
              {cardsDe(s.pts)}
            </div>
          ))
        )}
        ...
        <div className="rodape">
          <strong>Nota:</strong> Os separadores (Diário/Admissão/Alta) e templates vêm do teu ficheiro de conteúdo. Gestor de separadores personalizados e sincronização chegam a seguir.
        </div>
      </div>
    </div>
  );
