...
// deck inicial: cópia dos cards do conteúdo + migração de versões anteriores
function deckInicial() {
  try {
    const antigos = JSON.parse(localStorage.getItem("medguia:cansaco:cards:proprios") || "[]");
    const favsAntigos = JSON.parse(localStorage.getItem("medguia:cansaco:cards:favs") || "[]");
    localStorage.removeItem("medguia:cansaco:cards:proprios");
    localStorage.removeItem("medguia:cansaco:cards:favs");
    const base = JSON.parse(JSON.stringify(dados.cards.defaults));
    base.forEach((c) => { c.origemTitle = c.title; if (favsAntigos.includes(c.title)) c.fav = true; });
    return [...base, ...antigos];
  } catch {
    const base = JSON.parse(JSON.stringify(dados.cards.defaults));
...
function Flashcards() {
  ...
  const ir = (d) => { setIdx((i) => (i + d + exemplos.length) % exemplos.length); setVirado(false); setExplica(false); };
...
// embutido (separador "Cards" do guia): cards rápidos. Vista própria: flashcards
// (aberta pela opção "Abrir flashcards" do modal do menu, como no original).
export default function CansacoCards({ accent = "#e85d4a", voltar, embutido = false }) {
  const aba = embutido ? 0 : 1;
  const [filtro, setFiltro] = useState("todas");
  const [deck, setDeck] = useEstadoLocal("medguia:cansaco:cards:deck", deckInicial());
  const [lixo, setLixo] = useEstadoLocal("medguia:cansaco:cards:lixo", []);
  const [catsExtra, setCatsExtra] = useEstadoLocal("medguia:cansaco:cards:cats-extra", []);
  ...
  const reporOriginal = () => {
    ...
    const original = dados.cards.defaults.find((x) => x.title === (atual.origemTitle || atual.title));
  ...
  return (
    <div className={embutido ? undefined : "ca ob-page"}>
      ...
      {embutido
        ? <div className="ob-section-label">{aba === 0 ? dados.cards.titulo : dados.flashcards.titulo}</div>
