...
import { estadoInicial, ageNum, fertil, genNote, quadLabel } from "./dor-abdominal-logica.js";

const { ageBuckets: AGE_BUCKETS, quadrantes: QUADRANTES, symCats: SYM_CATS, syms: SYMS, alms: ALMS, drgcls: DRGCLS, ants: ANTS, ui } = dados;
...
export default function DorAbdominalAnamnese({ voltar, embutido = false }) {
  ...
  const simNao = (campo) => (
    <div className="ca-an-flex">
      <Pill on={D[campo] === true} onClick={() => setD({ [campo]: D[campo] === true ? null : true })}>Sim</Pill>
  ...
  const passo0 = (
    <Passo titulo="Construir o caso clínico" sub="Define um perfil fictício para treinares" num="1">
      ...
      {D.hemodin === "instavel" && (
        <>
          <Html className="da-instavel" html={ui.intro.instavelHtml} />
          <Alerta html={ui.intro.instavelNota} />
        </>
      )}
    </Passo>
  );
  ...
  const passo1 = (
    <Passo titulo="Caracterização da dor" sub="Método SOCRATES — início, tipo, localização, modulação" num="2">
      ...
      <div className="da-quad-layout">
        <div>
          <div className="da-qgrid">
            {QUADRANTES.map((q) => (
              <button key={q.id} className={"da-qcell" + (has(D.loc, q.id) ? " on" : "")} onClick={() => setD((d) => ({ loc: tog(d.loc, q.id) }))}>
                {q.l}<span className="da-qf">{q.full}</span>
              </button>
            ))}
          </div>
          <div style={{ marginTop: 8 }}>
            <Pill on={D.difusa} onClick={() => setD((d) => ({ difusa: !d.difusa }))}>Dor difusa / mal localizada</Pill>
          </div>
        </div>
        <div className="da-quad-info">
          {QUADRANTES.map((q) => has(D.loc, q.id) ? <Alerta key={q.id} html={ui.caract.quadrantes[q.id]} /> : null)}
          {D.difusa && <Alerta tipo="r" html={ui.caract.difusa} />}
          {!D.loc.length && !D.difusa && <div className="ca-an-hint">{ui.caract.quadVazio}</div>}
        </div>
      </div>
      {grupoPills("Irradiação",
        [["dorso", "Dorso"], ["ombrod", "Ombro/escápula dta"], ["ombroe", "Ombro/escápula esq"], ["virilha", "Virilha / genitais"], ["retro", "Retroesternal"], ["flancos", "Flancos / cintura"]], "irrad", (
          <>{["dorso", "ombrod", "ombroe", "virilha", "retro", "flancos"].map((t) =>
  ...
  const passo15 = (
    <Passo titulo="Contexto desencadeante" sub="Raciocínio dirigido" cor="#ca8a04">
      ...
      <div className="ca-an-flex">
        ...
        <Pill on={D.desRec === "nao"} onClick={() => setD({ desRec: "nao", relRef: "", ainesRec: null, alcoolRec: null, movDes: null, menstr: "" })}>Não / não aplicável</Pill>
      </div>
      {D.desRec === "nao" && <Alerta html={ui.contexto.semCtx} />}
      {D.desRec === "sim" && (
        <>
          <Lbl>Relação temporal com a refeição</Lbl>
          <div className="ca-an-flex">
            {[["logoapos", "Logo após comer (minutos)"], ["gordura", "Após refeição gorda"], ["2_3h", "2-3h após comer"], ["jejum", "Em jejum / de madrugada"], ["sem", "Sem relação clara"]].map(([v, l]) => (
              <Pill key={v} on={D.relRef === v} onClick={() => setD({ relRef: v })}>{l}</Pill>
            ))}
          </div>
          {D.relRef === "logoapos" && <Alerta html={ui.contexto.relLogoapos} />}
          {D.relRef === "gordura" && <Alerta html={ui.contexto.relGordura} />}
          {(D.relRef === "2_3h" || D.relRef === "jejum") && <Alerta html={ui.contexto.relJejum} />}
          <Lbl>Uso recente de AINEs?</Lbl>
          {simNao("ainesRec")}
          {D.ainesRec === true && <Alerta tipo="r" html={ui.contexto.aines} />}
          <Lbl>Consumo recente de álcool (ou binge)?</Lbl>
          {simNao("alcoolRec")}
          {D.alcoolRec === true && <Alerta html={ui.contexto.alcool} />}
          <Lbl>Comportamento durante a dor</Lbl>
          <div className="ca-an-flex">
            <Pill on={D.movDes === "imobiliza"} onClick={() => setD({ movDes: D.movDes === "imobiliza" ? null : "imobiliza" })}>Fica imóvel (qualquer movimento dói)</Pill>
            ...
            <>
              ...
              <div className="ca-an-flex">
                {[["peri", "Peri-menstrual"], ["meio", "A meio do ciclo"], ["sem", "Sem relação"]].map(([v, l]) => (
                  <Pill key={v} on={D.menstr === v} onClick={() => setD({ menstr: v })}>{l}</Pill>
                ))}
              </div>
  ...
  const passo2 = (
    <Passo titulo="Sinais de alarme" sub="Pesquisar SEMPRE" num="3" cor="#dc2626">
      <Alerta tipo="r" html={ui.alm.intro} />
      ...
      {has(D.alm, "perdaponderal") && <Alerta html={ui.alm.perdaPonderal} />}
    </Passo>
  );

  const idadeK = D.idade;
  const condAge = (cond) => (cond === "young" && idadeK === "jovem") ||
    (cond === "old" && (idadeK === "meia" || idadeK === "idoso")) ||
    (cond === "postmeno" && D.sexo === "F" && (idadeK === "meia" || idadeK === "idoso"));
  const passo3 = (
  ...
  const passo4 = (
    <Passo titulo="Hábitos de vida" num="5">
      ...
      {togGrupo("Dieta", [["equilibrada", "Equilibrada"], ["gordura", "Rica em gordura"], ["pobrefibra", "Pobre em fibra"], ["restritiva", "Restritiva"]], "dieta", D.dieta === "gordura" ? ui.hab.dietaGordura : D.dieta === "pobrefibra" ? ui.hab.dietaPobreFibra : null)}
      ...
      {DRGCLS.map((cls) => {
        ...
        return (
          <div key={cls.cls} style={{ marginBottom: 12 }}>
            ...
            {cls.mech && (any || D[mid]) && <div className="ca-an-hint-box">{cls.mech}</div>}
            <div className="ca-an-flex">
  ...
  const passo6 = (
    <Passo titulo="Antecedentes" num="7">
      ...
      <div className="ca-an-flex">
        <Pill alarm on={D.hxFam.neo} onClick={() => setD((d) => ({ hxFam: { ...d.hxFam, neo: !d.hxFam.neo } }))}>Neoplasia digestiva</Pill>
        <Pill on={D.hxFam.dii} onClick={() => setD((d) => ({ hxFam: { ...d.hxFam, dii: !d.hxFam.dii } }))}>Doença inflamatória intestinal</Pill>
        <Pill on={D.hxFam.lit} onClick={() => setD((d) => ({ hxFam: { ...d.hxFam, lit: !d.hxFam.lit } }))}>Litíase biliar</Pill>
      </div>
      {(D.hxFam.neo || D.hxFam.dii || D.hxFam.lit) && (
  ...
  const passo7 = (
    <Passo titulo="Exame objectivo" sub="Se normal, não alterar" num="8">
      ...
      <EoSec t="Abdómen: quadrantes dolorosos à palpação">
        ...
        {D.rha === "aum" && <Alerta html={ui.eo.rhaAum} />}
        {D.rha === "dim" && <Alerta html={ui.eo.rhaDim} />}
        {D.rha === "aus" && <Alerta tipo="r" html={ui.eo.rhaAus} />}
        {D.rha === "met" && <Alerta tipo="r" html={ui.eo.rhaMet} />}
        <Lbl>Massa / organomegália</Lbl>
        <div className="ca-an-flex">
          {[["Massa palpável", true], ["Hepatomegália"], ["Esplenomegália"], ["Massa pulsátil", true], ["Globo vesical"]].map(([l, al]) => (
        ...
        {has(D.massa, "Hepatomegália") && <Alerta html={ui.eo.hepatomegalia} />}
        {has(D.massa, "Esplenomegália") && <Alerta html={ui.eo.esplenomegalia} />}
        {has(D.massa, "Massa pulsátil") && <Alerta tipo="r" html={ui.eo.massaPulsatil} />}
        {has(D.massa, "Globo vesical") && <Alerta html={ui.eo.globoVesical} />}
      </EoSec>
      ...
      <EoSec t="Toque rectal">
        <div className="ca-an-flex">
          {[["nf", "Não realizado"], ["nl", "Normal"], ["sangue", "Sangue vivo", true], ["melena", "Melenas", true], ["massa", "Massa", true], ["dor", "Dor / abaulamento", true]].map(([v, l, al]) => (
            <Pill key={v} alarm={al} on={D.tr === v} onClick={() => setD({ tr: D.tr === v ? null : v })}>{l}</Pill>
          ))}
        </div>
        {D.tr === "sangue" && <Alerta html={ui.eo.trSangue} />}
        {D.tr === "melena" && <Alerta tipo="r" html={ui.eo.trMelena} />}
        {D.tr === "massa" && <Alerta tipo="r" html={ui.eo.trMassa} />}
        {D.tr === "dor" && <Alerta html={ui.eo.trDor} />}
      </EoSec>
  ...
  return (
    <div className={embutido ? undefined : "ca da ob-page"}>
      {!embutido && <button className="ob-voltar" onClick={voltar}>‹ Menu Dor Abdominal</button>}
      {!embutido && <h1 className="ob-titulo">Treino de Anamnese — Dor Abdominal</h1>}
