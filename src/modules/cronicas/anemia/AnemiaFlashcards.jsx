...
import dados from "@conteudo/cronicas/anemia-flashcards.json";
...
// Flashcards da Anemia — fiel ao original: três baralhos por morfologia
// (Microcítica/Normocítica/Macrocítica), lista numerada, revisão (única ou
// aleatória) com flip, marcação certo/errado, toast e resultados.
// Sub-separador "Os meus": criação livre por baralho, guardada em localStorage.
// (Mesma mecânica do AsmaFlashcards, com classes an-*.)
export default function AnemiaFlashcards({ voltar, rotuloVoltar = "‹ Voltar" }) {
  const [deckId, setDeckId] = useEstadoLocal("medguia:anemia:flash:deck", dados.decks[0].id);
  const [sub, setSub] = useState("estudo");
  const [meus, setMeus] = useEstadoLocal("medguia:anemia:flash:meus", []);
  ...
  if (revisao) {
    ...
    return (
      <div className="ca anemia ob-page an-flash">
            ...
            <div className={"ca-flash-scene" + (virado ? " flipped" : "") + (flash ? " " + flash : "")} onClick={() => setVirado((v) => !v)}>
              <div className="ca-flash-inner">
                ...
                <div className="ca-flash-face ca-flash-back">
                  ...
                  <div className="an-flash-stats">
                    <span className="an-stat ok"><Ico name="check" s={13} />{revisao.scoreR}</span>
                    <span className="an-stat err"><Ico name="close" s={13} />{revisao.scoreW}</span>
                    <span className="an-stat pct">{pct}%</span>
                  </div>
                </div>
              </div>
            </div>

            <div className="an-flash-controls">
              <button className="an-ctrl" title="Anterior" onClick={() => ir(-1)}>‹</button>
              <button className="an-ctrl errado" title="Errei" onClick={() => marcar("wrong")}><Ico name="close" s={16} /></button>
              <div className="an-score-pill"><span className="err">{revisao.scoreW}</span><span className="ok">{revisao.scoreR}</span></div>
              <button className="an-ctrl certo" title="Acertei" onClick={() => marcar("right")}><Ico name="check" s={16} /></button>
              <button className="an-ctrl" title="Próximo" onClick={() => ir(1)}>›</button>
            </div>

            {toast && <div className={"an-toast " + (toast.tipo === "right" ? "ok" : "err")}>{toast.texto}</div>}
          </>
        )}
      </div>
    );
  ...
  return (
    <div className="ca anemia ob-page an-flash">
      ...
      <div className="ob-sub an-flash-subsub">
        ...
        <>
          ...
          {formAberto && (
            <div className="ca-flash-form">
              ...
              <input className="campo ca-an-inp" value={q} onChange={(ev) => setQ(ev.target.value)} placeholder="Ex.: Ferritina < 30 — diagnóstico?" />
      ...
      {cartas.length === 0 ? (
        ...
        <>
          <button className="ca-btn an-start-btn" onClick={() => entrarRevisao(baralhar(cartas), 0, "random")}>
            Iniciar revisão aleatória ({cartas.length} {cartas.length === 1 ? "card" : "cards"})
          </button>
          <div className="an-fc-lista">
            {cartas.map((c, i) => (
              <div key={i} className="an-fc-item" onClick={() => entrarRevisao(cartas, i, "single")}>
                <span className="an-fc-num">{i + 1}</span>
                <span className="an-fc-texto">{c.q}</span>
                {sub === "meus"
                  ? <button className="an-fc-del" title="Apagar" onClick={(ev) => { ev.stopPropagation(); apagarMeu(c._i); }}>×</button>
                  : <span className="an-fc-seta">›</span>}
              </div>
            ))}
          </div>
        </>
